...
class AttentionRecognitionHead(nn.Module):
  ...
  def beam_search(self, x, beam_width, eos):
    ...
    for i in range(self.max_len_labels):
      ...
      state = state.index_select(1, predecessors.squeeze().long()) # ← Add .long() here
    ...
    while t >= 0:
      ...
      current_symbol = stored_emitted_symbols[t].index_select(0, t_predecessors.long()) # ← Add .long() here
      t_predecessors = stored_predecessors[t].index_select(0, t_predecessors.long()).squeeze() # ← Add .long() here
